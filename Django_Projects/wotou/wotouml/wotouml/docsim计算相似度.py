...
for item_text in raw_documents:
    # 原文档是一个list[str1，str2,```]
    item_str = list(jieba.cut(item_text,cut_all=False))
    item_str = remove_stopwords(item_str)
    corpora_documents.append(item_str)
    # 处理后得到list[list1[分词后的字符串1，字符串2],list2[]]
...
# 生成语料,将处理后的文档list中的每一个文档，用字典中的token的id构成向量表示出来
corpus = [dictionary.doc2bow(text) for text in corpora_documents]
print('corpus',type(corpus),np.shape(corpus),corpus)
# (16,),list[list1[],list2[],```]

similarity = Similarity('-Similarity-index', corpus, num_features=500)
...
# similarity.num_best = 4
# 如果num_best属性没有set一个具体值的话，会返回一个相似度值的列表[相似度1,相似度2，···]
#如果设置了一个具体值的话，返回[(index_of_document, similarity),···]
print(similarity[test_corpus_1])  # 返回最相似的样本材料


# 将文档内容换成数据库中的专利内容后，运行时程序会出现 python 已停止工作！！！
# 该问题尚未解决