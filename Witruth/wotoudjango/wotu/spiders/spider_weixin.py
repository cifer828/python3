...
def get_one_page_content(href, times=0):
    ...
    # 如果不是分享文章，直接提取正文
        ...
        try:
            ...
            print (type(wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="js_content"]/p')))))
...
def search_mongodb(title, times=0):
    """判断文章是否已经存在于mongodb中"""
    try:
        ret = db.weixin.find_one({'name': url})
        if not ret:
            return True
        else:
            return False
        ...
        for item in items:
            ...
            for subitem in subitems:
                ...
                if search_mongodb(title):
                    ...
                    spidertime = datetime.datetime.now().strftime('yyyy-mm-dd')
                    # 内容不为空则执行插入
                    if content != '':
